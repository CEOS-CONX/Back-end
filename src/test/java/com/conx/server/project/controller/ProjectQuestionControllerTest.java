...
class ProjectQuestionControllerTest {
    ...
    @Test
    @DisplayName("프로젝트 질문 목록을 기본 조건으로 조회한다")
    void getQuestions() throws Exception {
        // given
        LocalDateTime createdAt =
                LocalDateTime.of(2026, 7, 12, 14, 30);

        ProjectQuestionResponse question =
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.message")
                        .value("프로젝트 질문 목록 조회에 성공했습니다."))
                .andExpect(jsonPath("$.payload.content[0].questionId")
                        .value(QUESTION_ID))
                .andExpect(jsonPath("$.payload.content[0].writerName")
                        .value("김크루"))
                .andExpect(jsonPath("$.payload.content[0].content")
                        .value("프로젝트 결과물 형식이 궁금합니다."))
                .andExpect(jsonPath("$.payload.content[0].secret")
                        .value(false))
                .andExpect(jsonPath("$.payload.content[0].canView")
                        .value(true))
                .andExpect(jsonPath("$.payload.content[0].answered")
                        .value(false))
        ...
        verify(projectQuestionService).getQuestions(
                PROJECT_ID,
                0,
                10,
                false,
    ...
    @Test
    @DisplayName("내가 작성한 프로젝트 질문만 조회한다")
    void getMyQuestions() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.payload.number").value(1))
                .andExpect(jsonPath("$.payload.size").value(5))
